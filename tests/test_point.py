...
def generate_cluster_points(center, num_points=1000, noise_std=0.0, outlier_fraction=0.0):
    ...
    if outlier_fraction > 0:
        ...
        points[outlier_indices] = np.random.uniform(-5, 5, (num_outliers, 3)).astype(np.float32)
...
def test_point_fit_noisy_data(device):
    ...
    assert torch.allclose(result.center, torch.tensor(true_center, device=device), atol=0.5)
...
def test_point_fit_performance(device):
    points = generate_cluster_points(np.array([0., 0., 0.]), 
                                  num_points=10000, noise_std=0.1)
    points = points.to(device)
