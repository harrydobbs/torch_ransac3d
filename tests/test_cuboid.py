...
def generate_cuboid_points(center, dimensions, rotation_matrix, num_points=1000, noise_std=0.0, outlier_fraction=0.0):
    ...
    for normal, d in faces:
        ...
        if normal[0] != 0:  # x-face
            y = np.random.uniform(-dimensions[1]/2, dimensions[1]/2, points_per_face)
            z = np.random.uniform(-dimensions[2]/2, dimensions[2]/2, points_per_face)
            x = np.full_like(y, d)
            face_points = np.column_stack((x, y, z))
        elif normal[1] != 0:  # y-face
            x = np.random.uniform(-dimensions[0]/2, dimensions[0]/2, points_per_face)
            z = np.random.uniform(-dimensions[2]/2, dimensions[2]/2, points_per_face)
            y = np.full_like(x, d)
            face_points = np.column_stack((x, y, z))
        else:  # z-face
    ...
    if outlier_fraction > 0:
        ...
        points[outlier_indices] += np.random.uniform(-max_dim, max_dim, (num_outliers, 3))
