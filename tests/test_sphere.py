...
def generate_sphere_points(centre, radius, num_points=1000, noise_std=0.0, outlier_fraction=0.0):
    ...
    if outlier_fraction > 0:
        ...
        points[outlier_indices] += np.random.uniform(-2*radius, 2*radius, (num_outliers, 3)).astype(np.float32)
...
def test_sphere_fit_perfect_sphere(device):
    ...
    assert torch.allclose(result.center, torch.tensor(true_center, device=device), atol=0.1)
...
def test_sphere_fit_noisy_data(device):
    ...
    assert torch.allclose(result.center, torch.tensor(true_center, device=device), atol=0.2)
...
def test_sphere_fit_with_outliers(device):
    ...
    true_radius = 1.5
    points = generate_sphere_points(true_center, true_radius, 
                                  noise_std=0.02, outlier_fraction=0.3)
    points = points.to(device)
...
def test_sphere_fit_edge_cases(device):
    ...
    result = sphere_fit(points, thresh=0.1, device=device)
    assert len(result.inliers) >= 0  # Any number of inliers is acceptable for degenerate case

def test_sphere_fit_performance(device):
    points = generate_sphere_points(np.array([0., 0., 0.]), 1.0, 
                                  num_points=10000, noise_std=0.01)
    points = points.to(device)
