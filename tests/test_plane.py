...
def generate_plane_points(normal, d, num_points=1000, noise_std=0.0, outlier_fraction=0.0):
...
def test_plane_fit_perfect_plane(device):
    ...
    assert torch.allclose(equation[:3].abs(), torch.tensor(normal, device=device).abs(), atol=0.1)
    assert len(result.inliers) > 900

def test_plane_fit_noisy_data(device):
    normal = np.array([1., 1., 1.], dtype=np.float32) / np.sqrt(3).astype(np.float32)
    ...
    equation = result.equation / torch.norm(result.equation[:3])
    assert torch.allclose(equation[:3].abs(), torch.tensor(normal, device=device).abs(), atol=0.2)
    assert len(result.inliers) > 700

def test_plane_fit_with_outliers(device):
    normal = np.array([0., 1., 1.], dtype=np.float32) / np.sqrt(2).astype(np.float32)
